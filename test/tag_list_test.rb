...
class TagListTest < Test::Unit::TestCase
  ...
  test "TagList.from leaves arguments unchanged" do
    tags = '"One  ", Two'
    original = tags.dup
    TagList.from(tags)
  ...
  test "TagList.from with a single tag name" do
    assert_equal %w(Fun), TagList.from('Fun')
  end

  test "TagList.from with a single quoted tag name" do
    assert_equal %w(Fun), TagList.from('"Fun"')
  end

  test "TagList.from with a single blank tag name" do
    assert_equal [], TagList.from(nil)
    assert_equal [], TagList.from('')
  end

  test "TagList.from with tags contained in quoted tags" do
    assert_equal ['foo bar', 'bar'], TagList.from('"foo bar" bar')
  end

  test "TagList.from with a single quoted tag name that includes a comma (with comma delimiter)" do
    TagList.delimiter = ','
    assert_equal ['with, comma'], TagList.from('"with, comma"')
  end

  test "TagList.from does not delineate spaces (with comma delimiter)" do
    TagList.delimiter = ','
    assert_equivalent ['A B', 'C'], TagList.from('A B, C')
  end

  test "TagList.from with multiple tags" do
    assert_equivalent %w(Alpha Beta Delta Gamma), TagList.from('Alpha Beta Delta Gamma')
  end

  test "TagList.from with multiple tags with quotes and multiple spaces" do
    assert_equivalent %w(Alpha Beta Delta Gamma), TagList.from('Alpha  "Beta"  Gamma   "Delta"')
  end

  test "TagList.from with multiple tags with single quotes" do
    assert_equivalent ['A B', 'C'], TagList.from("'A B' C")
  end

  test "TagList.from with multiple tags with quotes and commas (with comma delimiter)" do
    TagList.delimiter = ','
    assert_equivalent ['Alpha, Beta', 'Delta', 'Gamma, something'], TagList.from('"Alpha, Beta", Delta, "Gamma, something"')
  end

  test "TagList.from removes leading/trailing whitespace from tag names" do
    assert_equivalent %w(Alpha Beta), TagList.from('" Alpha   " "Beta  "')
    assert_equivalent %w(Alpha Beta), TagList.from('  Alpha  Beta ')
  end

  test "TagList.from removes duplicate tags" do
